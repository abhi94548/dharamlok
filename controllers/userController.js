        ...
        try{
            jwt.verify(req.headers.token, 'bootspider', async function(err, user){
                if (err) res.status(400).json({success : false,message: err.message});
                    ...
                    let vendorDetails = await userModel.find({_id : req.body.id}).select("name").select("email").select("phone").select("address").select("profileImageUrl").select("userType").select("typeVendor");
